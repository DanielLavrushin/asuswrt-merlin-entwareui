...
const EntwareAction = {
  INSTALLED_PACKAGES: "entwareui_packages_installed"
};
class EntwarePackage {
  public name: string = "";
  public version: string = "";
}
class EntwareResponse {
  public installed?: EntwarePackage[] = [];
}
class EngineResponse {
  public entware?: EntwareResponse;
...
class Engine {
    ...
    return new Promise((resolve) => {
      ...
      if (payload) {
        ...
        chunks.forEach((chunk: string, idx) => {
          window.entware.custom_settings[`eui_payload${idx}`] = chunk;
        });

        const customSettings = JSON.stringify(window.entware.custom_settings);
  ...
  public async getResponse(): Promise<EngineResponse> {
    const response = await axios.get<EngineResponse>("/ext/entwareui/response.json");
...
export { EntwareAction, EntwarePackage, EntwareResponse, EngineResponse, EngineLoadingProgress };
